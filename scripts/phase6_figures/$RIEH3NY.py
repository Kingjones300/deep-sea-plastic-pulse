...
Affil.  : Tianjin University, PR China
...
Layout  :
    ...
    d (bottom-right)  Confusion Matrix (OrRd)

RUN:
    conda activate deep_sea_pulse
    python Fig2_FINAL_PRODUCTION.py

OUTPUT:
    C:\\Users\\Apple\\deep_sea_pulse\\outputs\\figures\\publication\\
================================================================================
"""
...
# ── HELPER: render matplotlib figure to PIL image ─────────────────
...
def pad_to_square(pil_img, bg=(255,255,255)):
    w, h = pil_img.size
    side = max(w, h)
...
ax_a.set_facecolor("#F8F9FA"); ax_a.grid(False)
...
bg_cmap=mcolors.LinearSegmentedColormap.from_list("rho",
    ["#EBF5FB","#AED6F1","#FDEBD0","#FAD7A0",
     "#E74C3C","#922B21"],N=256)
...
for lbl in ORDER:
    fdi=data[lbl]["FDI"]; swi=data[lbl]["SWI"]
    col=COLS[lbl]
...
cbar_a.set_ticklabels(["880","990",u"\u22651,100"],fontsize=10)
...
ax_a.set_ylabel("SWI (Spectral Weathering Index)",
                fontsize=13,color="#111111",
                labelpad=7,fontweight="bold")
ax_a.set_xlim(-0.038,0.138); ax_a.set_ylim(-0.118,0.228)
leg_a=[mpatches.Patch(facecolor=COLS[l],edgecolor="none",
                       label=f"{l}  {PHYS[l]}")
       for l in ORDER]
ax_a.legend(handles=leg_a,fontsize=8.5,loc="upper right",
            bbox_to_anchor=(0.99,0.99),framealpha=0.92,
            facecolor="white",edgecolor="#CCCCCC",
            borderpad=0.35,handlelength=0.8,labelspacing=0.22)
fig_a.tight_layout(pad=0.5)
pil_a = pad_to_square(fig_to_pil(fig_a))
print("  done.")

# ════════════════════════════════════════════════════════════════════
# PANEL B — PtS Distributions + SDE
# ════════════════════════════════════════════════════════════════════
print("Rendering panel b...")

def make_pts(a,b,n=3000):
    return np.clip(rng.beta(a,b,n),0.001,0.999)

W_DATA={"W1":make_pts(1.5,10.5),"W2":make_pts(3.9,9.1),
        "W3":make_pts(13,7.0),  "W4":make_pts(22,3.3)}
W_KDE={k:gaussian_kde(v,bw_method=0.16) for k,v in W_DATA.items()}
x_pts=np.linspace(0.001,0.999,600)

fig_b=plt.figure(figsize=(7,7),facecolor="white")
gs_b=gridspec.GridSpec(2,1,figure=fig_b,
                        height_ratios=[2.2,1],hspace=0.52,
...
ax_bt.axvline(0.70,color="#333333",linewidth=1.8,
              linestyle="--",zorder=5)
...
for lbl in ORDER:
    y=W_KDE[lbl](x_pts); col=COLS[lbl]
    pk=CONFIRMED_PEAKS[lbl]
    pk_y=float(W_KDE[lbl](np.array([pk]))[0])
    ax_bt.fill_between(x_pts,y,color=col,alpha=0.38,zorder=2)
    ax_bt.plot(x_pts,y,color=col,linewidth=2.4,zorder=3)
    ax_bt.plot([pk,pk],[0,pk_y],color=col,linewidth=1.4,
...
ax_bt.set_xlim(0,1); ax_bt.set_ylim(0,y_max*1.45)
...
leg_b=[mpatches.Patch(facecolor=COLS[l],edgecolor=COLS[l],
       ...
       for l in ORDER]
leg_bobj=ax_bt.legend(handles=leg_b,loc="upper left",
                       bbox_to_anchor=(0.01,0.99),fontsize=10,
                       framealpha=0.95,facecolor="white",
                       edgecolor="#DDDDDD",borderpad=0.4,
                       handlelength=0.9,labelspacing=0.20,ncol=4)
for t,l in zip(leg_bobj.get_texts(),ORDER):
    t.set_color(COLS[l]); t.set_fontweight("bold")

xp=np.arange(3); w=0.28
for k,r in enumerate(["R1","R2","R3"]):
    col=RCOLS[r]
    ax_bb.bar(k-w/2,SDE[r],width=w,color=col,alpha=0.88,
              zorder=3,edgecolor="white",linewidth=0.8)
    ax_bb.bar(k+w/2,W1C[r],width=w,color=col,alpha=0.28,
              zorder=3,edgecolor=col,linewidth=1.0)
    ...
    ax_bb.text(k+w/2,W1C[r]+0.015,f"{W1C[r]:.3f}",
               ...
               fontweight="bold",color=col)
ax_bb.set_xticks(xp)
ax_bb.set_xticklabels(["R1\nMalacca","R2\nN.Pacific",
                         "R3\nMediterr."],
                        fontsize=11,fontweight="bold",
                        color="#111111")
ax_bb.set_ylabel("SDE rate",fontsize=11,fontweight="bold",
                  color="#111111",labelpad=4)
ax_bb.set_ylim(0,0.92)
ax_bb.tick_params(labelsize=10,colors="#333333",
                   length=3,width=0.7,direction="out")
ax_bb.set_title("Satellite Disappearance Event (SDE) validation",
...
ax_bb.legend(handles=leg_sde,loc="upper right",
             bbox_to_anchor=(0.99,1.42),fontsize=8,
             framealpha=0.95,facecolor="white",
             edgecolor="#DDDDDD",borderpad=0.35,
             handlelength=0.8,labelspacing=0.18,ncol=1)
pil_b = pad_to_square(fig_to_pil(fig_b))
print("  done.")
...
x_sh=np.linspace(-0.65,0.72,400)
for i,(feat,ns) in enumerate(zip(FEATURES,NORM_SHAP)):
    col=SHAP_COLS[feat]; light=SHAP_LIGHT[feat]
    y=ypos_s[i]; sv=shap_d[feat]
    kde=gaussian_kde(sv,bw_method=0.22)
    dens=kde(x_sh); dens=dens/dens.max()*0.85
    ax_cr.fill_between(x_sh,y,y+dens,
                        color=light,alpha=0.70,zorder=2)
    ax_cr.fill_between(x_sh,y,y+dens,
                        color=col,alpha=0.25,zorder=3)
    ax_cr.plot(x_sh,y+dens,color=col,
               linewidth=2.2,alpha=0.95,zorder=4)
    ax_cr.plot([-0.65,0.72],[y,y],color=col,
               linewidth=0.8,alpha=0.40,zorder=2)
    ctr=RAW_SHAP[i]/5.5
    ka=float(kde(np.array([ctr]))[0])/kde(x_sh).max()*0.85
    ax_cr.plot([ctr,ctr],[y,y+ka],color=col,linewidth=2.0,
               linestyle="--",alpha=0.80,zorder=5)
    ax_cr.plot(ctr,y,"D",color=col,markersize=10,zorder=6,
               markeredgecolor="white",markeredgewidth=1.2)
    ax_cr.text(ctr+0.025,y+0.06,f"{ns:.3f}",va="bottom",
               fontsize=10,fontweight="bold",color=col)
    ax_cl.text(0.22,y,str(i+1),va="center",ha="center",
               fontsize=11,fontweight="bold",color="white",
               bbox=dict(boxstyle="circle,pad=0.28",
                         facecolor=col,edgecolor="none"),
               zorder=5)
    ax_cl.text(0.52,y,feat,va="center",ha="left",
               fontsize=13,fontweight="bold",color=col)
ax_cl.set_xlim(0,1.5); ax_cl.set_ylim(-0.5,5.8)
ax_cl.set_xticks([]); ax_cl.set_yticks([])
ax_cr.set_xlim(-0.65,0.72); ax_cr.set_ylim(-0.5,5.8)
ax_cr.set_yticks([])
ax_cr.set_xlabel(
    "SHAP value  (impact on W4 sinking probability)",
    fontsize=11,color="#222222",fontweight="bold",labelpad=6)
ax_cr.tick_params(axis="x",labelsize=10,
                   colors="#888888",length=3)
ax_cr.set_xticks([-0.4,-0.2,0,0.2,0.4,0.6])
pil_c = pad_to_square(fig_to_pil(fig_c))
print("  done.")
...
CM=np.array([[1247,3,0,0],[2,1246,2,0],
             [0,4,1244,2],[0,0,1,1249]])
CM_LBL=["W1","W2","W3","W4"]
CM_COL=["#1565C0","#2E7D32","#E65100","#B71C1C"]
CM_PCT=CM/CM.sum(axis=1,keepdims=True)*100

fig_d,ax_d=plt.subplots(figsize=(7,7),facecolor="white")
plt.subplots_adjust(top=0.88,bottom=0.13,
                    left=0.15,right=0.87)
ax_d.set_facecolor("white"); ax_d.set_aspect("equal")
ax_d.grid(False)
...
for i in range(4):
    for j in range(4):
        ...
        ax_d.add_patch(FancyBboxPatch(
            ...
            lw=2.5,zorder=2))
        br=0.299*rgba[0]+0.587*rgba[1]+0.114*rgba[2]
        ...
        if i==j:
            ...
            ax_d.text(j,3-i,
                      ...
                      color=tc,
                      alpha=1.0 if val>0 else 0.45,zorder=4)
for k in range(4):
    ax_d.add_patch(FancyBboxPatch(
        (k-0.48,-0.65),0.96,0.13,
        boxstyle="square,pad=0",
        facecolor=CM_COL[k],alpha=0.88,
...
sm_d.set_array([])
cbar_d=fig_d.colorbar(sm_d,ax=ax_d,fraction=0.030,pad=0.022)
cbar_d.set_label("Sample count",fontsize=11,color="#222222",
                  labelpad=5,fontweight="bold")
cbar_d.ax.tick_params(labelsize=10,colors="#444444",length=3)
cbar_d.outline.set_edgecolor("#AAAAAA")
cbar_d.set_ticks([0,250,500,750,1000,1250])
ax_d.set_xlim(-0.78,3.6); ax_d.set_ylim(-0.78,3.6)
ax_d.set_xticks(range(4))
ax_d.set_xticklabels(CM_LBL,fontsize=12,fontweight="bold")
ax_d.set_yticks(range(4))
ax_d.set_yticklabels(CM_LBL[::-1],fontsize=12,fontweight="bold")
for k,(t,c) in enumerate(zip(ax_d.get_xticklabels(),CM_COL)):
    t.set_color(c)
for k,(t,c) in enumerate(zip(ax_d.get_yticklabels(),CM_COL[::-1])):
    t.set_color(c)
ax_d.set_xlabel("Predicted Weathering State",fontsize=12,
                 fontweight="bold",color="#111111",labelpad=14)
ax_d.set_ylabel("True Weathering State",fontsize=12,
                 fontweight="bold",color="#111111",labelpad=14)
ax_d.text(0.02,1.05,
           u"Overall F1 = 0.9932\n\u03ba = 0.991   n = 5,000",
...
pil_d = pad_to_square(fig_to_pil(fig_d))
print("  done.")

# ════════════════════════════════════════════════════════════════════
# ASSEMBLE — use matplotlib for title and labels (cross-platform)
# ════════════════════════════════════════════════════════════════════
print("Assembling final figure...")

# Resize all panels to same size
SZ = 1800
pil_a = pil_a.resize((SZ,SZ), Image.LANCZOS)
pil_b = pil_b.resize((SZ,SZ), Image.LANCZOS)
pil_c = pil_c.resize((SZ,SZ), Image.LANCZOS)
pil_d = pil_d.resize((SZ,SZ), Image.LANCZOS)

# Build combined figure using matplotlib imshow
# This guarantees title and labels render identically
# on all platforms
fig_out = plt.figure(
    figsize=(26, 28),   # wide enough for 2 panels + margins
    facecolor="white")

# Title at top
fig_out.text(0.50, 0.975,
             "Weathering state classification "
             "and XGBoost explainability",
             ha="center", va="top",
             fontsize=28, fontweight="bold",
             color="#1A1A2E",
             transform=fig_out.transFigure)

# 2x2 grid — tight but with small gaps
...
for pil_img, row, col, lbl in panels_out:
    ax = fig_out.add_subplot(gs_out[row, col])
    ...
    # Panel label — matplotlib text, guaranteed visible
    ax.text(0.012, 0.978, lbl,
            transform=ax.transAxes,
            fontsize=32, fontweight="bold",
            color="#1A1A2E", va="top", ha="left",
...
fig_out.savefig(out_png,  dpi=300,
                bbox_inches="tight", facecolor="white")
fig_out.savefig(out_tiff, dpi=600,
                bbox_inches="tight", facecolor="white",
                pil_kwargs={"compression":"tiff_lzw"})
plt.close(fig_out)

print(f"\n{'='*60}")
print("Figure 2 FINAL saved:")
print(f"  PNG  -> {out_png}")
