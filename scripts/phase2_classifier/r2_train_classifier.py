...
OUT_DIR = ROOT / "outputs" / "results" / "phase2" / "R2"
...
# R2 oligotrophic parameters — slow biofouling, aged gyre plastic
...
print("\n[1/5] Generating R2 training data (oligotrophic parameterisation)...")
rng = np.random.default_rng(42)
records = []

# (FDI_mean, FDI_std, SWI_mean, SWI_std, NDVI_mean, NDVI_std, SNR_mean, SNR_std, n_samples)
# R2: W1 and W2 dominate (1600 each), W3/W4 less common (1100 each) — reflects gyre reality
params = {
...
try:
    ...
    plt.title("SHAP Feature Importance - W4 Critical Sinking State (R2 Gyre)")
    plt.tight_layout()
...
# 5. Save outputs
print("\n[5/5] Saving outputs...")
model.save_model(str(OUT_DIR / "xgboost_weathering_model_R2.json"))
...
ax.set_title("Confusion Matrix W1-W4 | R2 North Pacific Gyre")
for i in range(4):
    for j in range(4):
...
summary = {
    ...
    "oligotrophic_parameterisation": True,
